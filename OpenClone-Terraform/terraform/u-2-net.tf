...
# Define the ClusterIP service (internal communication)
 resource "kubernetes_service" "openclone-u-2-net-clusterip" {
   metadata {
     name = "openclone-u-2-net-clusterip"
   }
 
   spec {
     selector = {
       pod_id = "openclone-u-2-net-pod"  # Updated to match the pod label from the deployment
     }
 
     port {
       protocol    = "TCP"
       port        = var.openclone_u2net_port
       target_port =  var.openclone_u2net_port
     }
...
resource "kubernetes_deployment" "openclone-u-2-net" {
  depends_on = [
  ...
  spec {
    ...
    selector { match_labels = { pod_id = "openclone-u-2-net-pod" } } # must match pod (template.metadata) so kubernetes know which pods this deployment manages
    template {                                                       # this is the pod template 
      ...
      spec {
        container {
          ...
          port { container_port =  var.openclone_u2net_port }
